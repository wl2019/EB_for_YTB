...
namespace YTB
{
    public partial class frm_Qry_Prod : Form
    {
        string mszTitle = "商品名称        ,总净重(" + MyStart.gszWeight + "),总价(元),均价(元/" + MyStart.gszWeight + "),"
            + "买方服务费(元),卖方服务费(元),买方系统使用费(元),卖方系统使用费(元),销售总价(元)";
        ...
        private void frm_Qry_Prod_Load(object sender, EventArgs e)
        {
            ...
            if (iRst == 0)
            {
                ...
                if (iStoreNum > 0)
                    comboBox_Firm.Items.Add("0-所有商户");
            ...
            int iProdNum = 0;//商品
            string szSql = "select concat(PROD_ID,'-',PROD_NAME) from base_prod where PROD_LEVEL=1 order by PROD_ID";
            int iRst = MyStart.oMyDb.ReadData(szSql, "tableA", ref ds, ref szErr);
            if (iRst == 0)
            {
        ...
        private void WriteDataToGrid(string szFirmName, string szFirmCard, string szDateCondition, ref int iBgnRow)
        {
            string szSql = "select PROD_NAME,format(sum(prod_w - prod_nw) / 1000, 3),"
                ...
                + "where a.CANCEL_MARK='N' and a.prod_id =" + szFirmCard + " and a.PROD_ID = b.PROD_ID and b.PROD_LEVEL=1 and " + szDateCondition
                + " group by a.prod_id";
            string szErr = "";
            DataSet ds = new DataSet();
            int iRst = MyStart.oMyDb.ReadData(szSql, "tableA", ref ds, ref szErr);
            if (iRst != 0)
            {
                ...
                MyIniFile.WriteLog("查询商品交易记录", "SQL=" + szSql + ",Err=" + szErr);
                MyStart.oMyDb.Close();
                return;
            }
        ...
        {
            MyFunc.WriteToDbLog("生成商品销售汇总报表", "", "MSG", MyStart.giUserID);

            string szErr = "";
            string szRptFle = "";
            bool bRst = MyFunc.ExportDataToFile(mszRptTitle, mszRptDate, "", "A,G,I", mszTitle, ref dataGridViewRst, miRows, "", "E:\\", "商品销售汇总报表" + DateTime.Now.ToString("yyyyMMdd"),
                ref szErr);
            if (bRst)
            {
                szRptFle = szErr;
